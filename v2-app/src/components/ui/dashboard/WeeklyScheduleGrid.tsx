...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
    ...
    closed:    { bg: '#f5f5f4', text: '#78716c', border: '#a8a29e' }
}
...
export default function WeeklyScheduleGrid({ sessionList, role }: WeeklyScheduleGridProps) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-white-border p-5">
            {/* Header */}
            ...
            <div className="flex items-center gap-4 mb-4 flex-wrap">
                {Object.entries(STATUS_COLORS).map(([status, colors]) => (
                    <div key={status} className="flex items-center gap-1.5 text-xs font-semibold capitalize">
                        <span className="w-2.5 h-2.5 rounded-full border" style={{ backgroundColor: colors.bg, borderColor: colors.border }} />
                        {status.replace('_', ' ')}
                    </div>
                ))}
            </div>
