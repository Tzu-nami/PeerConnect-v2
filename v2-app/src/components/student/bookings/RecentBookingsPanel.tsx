...
export default function RecentBookingsPanel({ bookings }: RecentBookingsPanelProps) {
    ...
    return (
        <div className="bg-cream rounded-xl border border-cream-border overflow-hidden mb-7">
            ...
            >
                <span className="font-bold text-lg text-text-brown">Recent Bookings</span>
            ...
            {open && (
                <div className="px-5 pb-5">
                    {bookings.length === 0 ? (
                        <p className="text-sm text-text-brown-light text-center py-4">No recent bookings found.</p>
                    ) : (
                        bookings.map((b, i) => (
                            <div key={b.id} className={`mb-4 pb-4 ${i < bookings.length - 1 ? 'border-b border-cream-border' : ''}`}>
                                <div className="flex items-start justify-between gap-3">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-text-brown">{b.subject_code.toUpperCase()}</p>
                                        <p className="text-xs font-medium text-text-brown-light mt-0.5 truncate">
                                            <span className="font-bold text-text-brown-light tracking-wide">Mentor:</span> {b.mentor_name || 'Any'}
                                        </p>
                                        <p className="text-xs font-medium text-text-brown-light mt-0.5 truncate">
                                            <span className="font-bold text-text-brown-light tracking-wide">Topic:</span> {b.topic}
                                        </p>
                                        <p className="text-xs font-medium text-text-brown-light mt-0.5 truncate">
                                            <span className="font-bold text-text-brown-light tracking-wide">Mode:</span> {b.mode}
                                        </p>
                                    </div>
                                ...
                                <p className="text-[11px] text-text-brown font-bold mt-1.5 flex items-center flex-wrap">
                                    <FaRegCalendar className="mr-1.5" />
                                    {new Date(b.date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', weekday: 'short' })}
                                    <span className="mx-1.5 text-text-brown-light">|</span>
