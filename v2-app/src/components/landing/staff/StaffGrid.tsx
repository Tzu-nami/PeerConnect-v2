...
export default function StaffGrid({ staffList }: {  staffList: StaffProfile[] }) {
    return (
        <section
            className="px-6 md:px-20 lg:px-40 xl:px-64 pb-20 border-t border-white-border pt-12 animate-fade-up [animation-delay:150ms]">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-10 items-stretch">
                {staffList.map((staff, index) => {
                    ...
                    return (
                        <div key={staff.id} className={`flex flex-col border border-white-border rounded-sm overflow-hidden 
                            ...
                            <div
                                className="flex flex-col flex-1 justify-center divide-y divide-white-border text-sm text-text-primary">
